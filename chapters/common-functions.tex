\chapter{Common Piecewise Functions}
...
\begin{proof}
    We can derive this representation of $\max\br{a,b}$ in much the same way as \autoref{example:algebra_1}:
    \begin{align*}
        \max\br{a,b} &=\begin{piecewise}
            a & a\geq b\\
            b & a\leq b
...
\section{Mixed maximum and minimum}
It is here we note some identities of $\max$ and $\min$ but also extend explicitly our ability to manipulate piecewise functions which would otherwise elude us, per \autoref{example:algebra_1}.
...
\begin{theorem}
    ...
    This can be shown using the same techniques found in \autoref{example:algebra_1}.
\end{theorem}

\begin{example}
    We wish to express the following function (similar to that found in \autoref{example:algebra_clamping}) in terms of step functions:
    $$
        f(x) = \begin{piecewise}
            x & x>1 \\
            1 & -1<x<1 \\
...
\section{The floor and ceiling functions}
...
\section{The modulo operation/function}
...
\section{Characteristic functions and Iverson brackets}
